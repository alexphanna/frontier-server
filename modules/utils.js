import { points, buildings, game, getPlayerArray } from '../server.js';
import { Vertex, Edge, Tile } from './geometry.js';
import Player, { PublicPlayer } from './player.js';
 ...
 */
export function countRoad(row, col, player, prevEdgeRow = -1, prevEdgeCol = -1) {
    let nextRoads = [];
...
export function broadcastPoints() {
    for (let i = 0; i < game.players.size; i++) {
        if (game.turn >= game.players.size && game.round < 2 && getPlayerArray()[i].prevVertex != null) {
            var playerEdges = [];
            ...
            let temp = Vertex.adjacentEdges(getPlayerArray()[i].prevVertex[0], getPlayerArray()[i].prevVertex[1]);
            ...
            for (let j = 0; j < points.roadEdges.length; j++) {
                playerEdges.push([]);
                for (let k = 0; k < points.roadEdges[j].length; k++) {
                    playerEdges[j].push(points.roadEdges[j][k] == i);
                }
            }
        }
        ...
        for (let j = 0; j < points.cityVertices.length; j++) {
            ...
            for (let k = 0; k < points.cityVertices[j].length; k++) {
                playerCityVertices[j].push(points.cityVertices[j][k] == i);
            }
        }
        ...
        for (let j = 0; j < points.settlementVertices.length; j++) {
            ...
            for (let k = 0; k < points.settlementVertices[j].length; k++) {
                if (game.round < 2) {
                    ...
                    playerSettlementVertices[j].push(points.settlementVertices[j][k] == i);
                }
            }
        }
        Array.from(game.clients)[i].send('vertices settlement ' + JSON.stringify(playerSettlementVertices));
        Array.from(game.clients)[i].send('vertices city ' + JSON.stringify(playerCityVertices));
        Array.from(game.clients)[i].send('edges ' + JSON.stringify(playerEdges));
    }
}
            ...
            if (args[0] === 'add') {
                    ...
                    if (player.name !== args[2]) {
                        for (let i = 0; i < game.players.size; i++) {
                            if (Array.from(game.players)[i].name === player.name) {
                                Array.from(game.clients)[i].send(`trade domestic ${args[2]} ${JSON.stringify(you)} ${args[4]}`);
                            }
                        }
                ...
                if (args[1] === 'settlement') {
                    ...
                    if (game.round < 2) {
                        ...
                        // check if settlement is legal
                        if (points.settlementVertices[row][col] != game.turn % game.players.size) {
                            ws.send('error Illegal settlement placement');
                            return;
                        }
                    ...
                    if (game.round === 1) {
                        ...
                        if (points.roadEdges[row][col] != game.turn % game.players.size) {
                            ws.send('error Illegal road placement');
                            return;
                        }
                    }
                    ...
                    for (let i = 0; i < adjacentVertices.length; i++) {
                        ...
                        for (let j = 0; j < adjacentEdges.length; j++) {
                            if (isNaN(points.roadEdges[adjacentEdges[j][0]][adjacentEdges[j][1]])) {
                                if (game.round === 1) {
